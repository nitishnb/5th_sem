...
def dfs(src, target, limit, visited_states):
    ...
    for action in actions:
        if action not in visited_states and iddfs(action, target, limit) < min:
            min = iddfs(action, target, limit)
            new_move = action
    print("Stage : ", limit+1)
    display(new_move)
    if dfs(new_move, target, limit+1, visited_states) is True:
        return True
    return False
